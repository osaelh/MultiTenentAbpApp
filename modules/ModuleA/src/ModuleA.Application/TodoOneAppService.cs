...
namespace ModuleA
{
    public class TodoOneAppService : ModuleAAppService
    {
        ...
        {
            var todo = new TodoOneDto(todoOneDto.Content, todoOneDto.IsDone, CurrentTenant.Id);
            var mappedTodoOne = ObjectMapper.Map<TodoOneDto, TodoOne>(todo);
            var createdTodoOne = await todoOneRepository.InsertAsync(mappedTodoOne);
